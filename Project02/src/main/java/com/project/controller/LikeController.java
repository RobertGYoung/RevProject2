...
public class LikeController {
	...
	public ResponseEntity<Like> getLikeById(@PathVariable(value="id") Long likeId) throws ResourceNotFoundException{
		...
		Like like = likeRepo.findById(likeId) .orElseThrow(() -> new ResourceNotFoundException("Like not found for this id :: " + likeId));
	...
	public Like createLike(@PathVariable(value = "userId") Long userId, @RequestBody Like like) throws ResourceNotFoundException {
		return likeService.createLike(userId, like);
	}
	
	//update is_like
    @PatchMapping("/likes/{id}")
    public ResponseEntity<Like> updateLike(@PathVariable(value = "id") Long likeId,
     @Valid @RequestBody Like likeDetail) throws ResourceNotFoundException {
    ...
    public Map<String, Boolean> deleteLike(@PathVariable(value = "id") Long likeId)
